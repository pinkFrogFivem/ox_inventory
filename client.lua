...
for _, v in pairs(Items --[[@as table<string, OxClientItem>]]) do
    ...
    if v.buttons then
        for i = 1, #v.buttons do
            local btn = v.buttons[i]
            table.insert(buttons, {
                label = btn.label,
                action = type(btn.action) == 'function' and btn.label or btn.action,
                group = btn.group
            })
        end
    end
...
RegisterNUICallback('changePaymentOption', function(data,cb)

	actualPaymentType = data.option
	cb('ok')
end)
