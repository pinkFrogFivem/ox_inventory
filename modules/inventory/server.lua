...
function OxInventory:openInventory(inv)
	...
	TriggerClientEvent('pinkFrog_syncClonePed',  self.id, true, inv)
end
...
function Inventory.SetSlot(inv, item, count, metadata, slot)
	...
	if currentSlot and newCount < 1 then
		...
		currentSlot = {name = item.name, label = item.label, weight = item.weight, slot = slot, count = newCount, description = item.description, metadata = metadata, stack = item.stack, close = item.close , rarity = item.rarity or 'common'}
	...
	for i = 1, #items do
		...
		if not item then
			...
			totalWeight = totalWeight + weight
				returnData[i] = {name = item.name, label = item.label, weight = weight, slot = i, count = count, description = item.description, metadata = metadata, stack = item.stack, close = item.close, rarity = item.rarity or 'common'}
		end
	end
	...
	if result then
		...
		for _, v in pairs(result) do
			...
			if item then
				...
				weight += slotWeight
				returnData[v.slot] = {name = item.name, label = item.label, weight = slotWeight, slot = v.slot, count = v.count, description = item.description, metadata = v.metadata, stack = item.stack, close = item.close, rarity = item.rarity or 'common'}
			end
		end
	end
...
function Inventory.AddItem(inv, item, count, metadata, slot, cb)
	...
	if not toSlot then
	   local minSlot, maxSlot, exceptionsKeys = exports['pinkFrog_inventoryAddon']:getMinMaxClothesSlotWithExceptions()
	   local itemKey = type(item) == "table" and item.name or tostring(item)
	   local isClothesItem = exceptionsKeys[itemKey] or false
		...
		if minSlot and maxSlot and not isClothesItem then
			if type(toSlot) == "number" then
				if toSlot >= minSlot and toSlot <= maxSlot then
					return false, 'inventory_full'
				end
			elseif type(toSlot) == "table" then
				for _, slotData in ipairs(toSlot) do
					if slotData.slot and slotData.slot >= minSlot and slotData.slot <= maxSlot then
						return false, 'inventory_full'
					end
				end
			end
...
function Inventory.CanCarryItem(inv, item, count, metadata)
    ...
    if item then
        ...
        if inv then
            local minSlot, maxSlot, exceptionsKeys = exports['pinkFrog_inventoryAddon']:getMinMaxClothesSlotWithExceptions()
            local itemKey = type(item) == "table" and item.name or tostring(item)
            local isClothesItem = exceptionsKeys and exceptionsKeys[itemKey] or false
    ...
    if table.type(items) == 'array' then
        for i = 1, #items do
            ...
            if item then
                ...
                totalWeight = totalWeight + weight
                inventory[data.slot] = {name = data.name, label = item.label, weight = weight, slot = data.slot, count = data.count, description = item.description, metadata = data.metadata, stack = item.stack, close = item.close, rarity = item.rarity or 'common'}
            end
        end
    end
