		...
		description = %q
	       ...
	       for _, item in pairs(dump) do
					if not ItemList[item.name] then
						...
						local itemStr = itemFormat:format(item.name, item.label, item.weight, item.stack, item.close,item.rarity,  item.description and json.encode(item.description) or 'nil')
...
function Items.Metadata(inv, item, metadata, count)
	...
	if metadata.imageurl and Utils.IsValidImageUrl then
		if Utils.IsValidImageUrl(metadata.imageurl) then
			Utils.DiscordEmbed('Valid image URL', ('Created item "%s" (%s) with valid url in "%s".\n%s\nid: %s\nowner: %s'):format(metadata.label or item.label, item.name, inv.label,item.rarity, metadata.imageurl, inv.id, inv.owner, metadata.imageurl), metadata.imageurl, 65280)
		else
			Utils.DiscordEmbed('Invalid image URL', ('Created item "%s" (%s) with invalid url in "%s".\n%s\nid: %s\nowner: %s'):format(metadata.label or item.label, item.name, inv.label,item.rarity, metadata.imageurl, inv.id, inv.owner, metadata.imageurl), metadata.imageurl, 16711680)
