	...
	if data.toType == 'player' then
		...
		if fromData then
			...
			if toData == nil or (fromItem.name == toItem?.name and fromItem.stack and table.matches(toData.metadata, metadata)) then
				...
				canAfford = canAffordItem(playerInv, currency, price, payment)
				elseif payment == 'bank' then 

					if exports['pinkFrog_inventoryAddon']:getFramework() == 'ESX' then
						local xPlayer = ESX.GetPlayerFromId(source)
						canAfford = xPlayer.getAccount('bank').money >= price
					elseif exports['pinkFrog_inventoryAddon']:getFramework() == 'QB' then
